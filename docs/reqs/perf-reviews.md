...
- [x] **HR configures review cycle** — period, cadence, and participants — *HR-facing UI under Grow → Performance System → "Review Cycles" tab; create/edit via modal with label, review type, period, self-assessment and manager deadlines; draft → open → closed status transitions; participants default to all active employees (explicit participant picker is a future enhancement)*
