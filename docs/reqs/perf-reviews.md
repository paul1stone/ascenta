...
- [ ] **Guardrail: no employee goes more than 90 days without a structured, documented conversation about their performance and development** — **NOT IMPLEMENTED: no enforcement logic; tracked in #46**
...
- [x] **3. Employee Strengths and Documented Contributions** — dedicated section documenting what the employee did well, specific contributions exceeding expectations, recognition-worthy behaviors aligned to organizational values; captures the "how" behind the "what" (collaborative contributions, creative problem-solving, positive impact on culture); primary input to compensation and promotion decisions — *dedicated "Strengths and documented contributions" block in `ManagerAssessmentForm` below the 10-category grid with `strengthsNarrative` (contributions exceeding expectations, unique capabilities) and `recognitionHighlights` (value-aligned behaviors); persisted on `managerAssessment.strengthsNarrative` and `managerAssessment.recognitionHighlights`; autosaved on blur. The employee-facing narrative (self-assessment strengths) is covered by PR #60 on the `selfAssessment` subdocument.*
