...
- [x] **Sidebar displays which strategic priorities the employee ladders to** during drafting — *`StrategyPrioritiesSidebar` component rendered alongside `ManagerAssessmentForm` in v2 flow; new `GET /api/grow/reviews/[id]/strategy-context` returns Foundation, in-scope strategy goals, and the employee's role translation (role contribution + alignment descriptors) via `getTranslationForEmployee()`*
