"use client";

import { useState } from "react";
import { cn } from "@ascenta/ui";
import { useAuth } from "@/lib/auth/auth-context";
import { LikertScale } from "./likert-scale";
import { Lock, CheckCircle2, Loader2, ArrowRight } from "lucide-react";

type ReflectManagerProps = {
  checkIn: any;
  onComplete: () => void;
};

type LikertDimension = {
  key: "clarity" | "recognition" | "development" | "safety";
  question: string;
  lowLabel: string;
  highLabel: string;
};

const DIMENSIONS: LikertDimension[] = [
  {
    key: "clarity",
    question:
      "How clear were you in communicating expectations and what success looks like?",
    lowLabel: "Not at all",
    highLabel: "Completely",
  },
  {
    key: "recognition",
    question:
      "How well did you recognize specific contributions and effort?",
    lowLabel: "Not at all",
    highLabel: "Completely",
  },
  {
    key: "development",
    question:
      "Was your focus on growth and future development, or on past performance?",
    lowLabel: "Past-focused",
    highLabel: "Growth-focused",
  },
  {
    key: "safety",
    question:
      "How safe do you think the employee felt being honest about challenges?",
    lowLabel: "Not safe",
    highLabel: "Fully safe",
  },
];

export function ReflectManager({ checkIn, onComplete }: ReflectManagerProps) {
  const { user } = useAuth();
  const [submitting, setSubmitting] = useState(false);
  const [submitError, setSubmitError] = useState<string | null>(null);

  const isCompleted = !!checkIn.managerReflect?.completedAt;

  const [scores, setScores] = useState<Record<string, number | null>>({
    clarity: checkIn.managerReflect?.clarity ?? null,
    recognition: checkIn.managerReflect?.recognition ?? null,
    development: checkIn.managerReflect?.development ?? null,
    safety: checkIn.managerReflect?.safety ?? null,
  });

  const [forwardAction, setForwardAction] = useState(
    checkIn.managerReflect?.forwardAction ?? "",
  );

  const allScoresSet = DIMENSIONS.every((d) => scores[d.key] !== null);
  const forwardActionFilled = forwardAction.trim().length > 0;

  const handleSubmit = async () => {
    if (!user || submitting || !allScoresSet || !forwardActionFilled) return;
    setSubmitting(true);
    setSubmitError(null);
    try {
      const res = await fetch(`/api/grow/check-ins/${checkIn._id}/reflect`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          "x-dev-user-id": user.id,
        },
        body: JSON.stringify({
          clarity: scores.clarity,
          recognition: scores.recognition,
          development: scores.development,
          safety: scores.safety,
          forwardAction: forwardAction.trim(),
        }),
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        setSubmitError(err.error ?? "Failed to submit reflection");
        return;
      }
      onComplete();
    } catch {
      setSubmitError("Failed to submit reflection");
    } finally {
      setSubmitting(false);
    }
  };

  // Read-only completed view
  if (isCompleted) {
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-2 rounded-lg border border-[#44aa99]/30 bg-[#44aa99]/5 px-4 py-3">
          <CheckCircle2 className="size-4 text-[#44aa99]" />
          <span className="text-sm font-medium text-[#44aa99]">
            Reflection submitted
          </span>
        </div>

        <div className="space-y-4">
          {DIMENSIONS.map((dim) => (
            <div key={dim.key} className="rounded-lg border bg-muted/20 p-4">
              <p className="text-sm text-muted-foreground mb-3">
                {dim.question}
              </p>
              <LikertScale
                value={scores[dim.key]}
                onChange={() => {}}
                lowLabel={dim.lowLabel}
                highLabel={dim.highLabel}
                disabled
              />
            </div>
          ))}
        </div>

        <div className="rounded-lg border bg-muted/20 p-4 space-y-2">
          <div className="flex items-center gap-1.5">
            <ArrowRight className="size-3.5 text-[#44aa99]" />
            <h4 className="text-sm font-semibold text-foreground">
              Forward Action
            </h4>
          </div>
          <p className="text-sm text-muted-foreground whitespace-pre-wrap">
            {forwardAction || "---"}
          </p>
        </div>
      </div>
    );
  }

  return (
    <div className="space-y-6">
      {/* Privacy banner */}
      <div className="flex items-start gap-2.5 rounded-lg border border-[#cc6677]/30 bg-[#cc6677]/5 px-4 py-3">
        <Lock className="size-4 mt-0.5 text-[#cc6677] shrink-0" />
        <p className="text-sm text-[#cc6677]">
          Your responses are private — your employee will not see them.
        </p>
      </div>

      {/* Likert dimensions */}
      <div className="space-y-4">
        {DIMENSIONS.map((dim) => (
          <div key={dim.key} className="rounded-lg border bg-white p-4">
            <p className="text-sm font-medium text-foreground mb-3">
              {dim.question}
            </p>
            <LikertScale
              value={scores[dim.key]}
              onChange={(v) =>
                setScores((prev) => ({ ...prev, [dim.key]: v }))
              }
              lowLabel={dim.lowLabel}
              highLabel={dim.highLabel}
            />
          </div>
        ))}
      </div>

      {/* Forward Action */}
      <div className="rounded-lg border bg-white p-4 space-y-3">
        <div className="flex items-center gap-1.5">
          <ArrowRight className="size-4 text-[#44aa99]" />
          <h3 className="text-sm font-semibold text-foreground">
            Forward Action
          </h3>
        </div>
        <p className="text-sm text-muted-foreground">
          This becomes the opening line of your next context briefing. What one
          thing will you focus on improving before the next check-in?
        </p>
        <textarea
          value={forwardAction}
          onChange={(e) => setForwardAction(e.target.value)}
          rows={3}
          className={cn(
            "w-full rounded-md border bg-muted/30 px-3 py-2 text-sm",
            "placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-[#44aa99]/30 focus:border-[#44aa99]",
          )}
          placeholder="Your forward action for next time..."
        />
      </div>

      {/* Submit error */}
      {submitError && (
        <p className="text-sm text-[#cc6677]">{submitError}</p>
      )}

      {/* Submit */}
      <button
        type="button"
        onClick={handleSubmit}
        disabled={submitting || !allScoresSet || !forwardActionFilled}
        className={cn(
          "w-full rounded-lg bg-[#44aa99] px-4 py-2.5 text-sm font-medium text-white transition-colors",
          "hover:bg-[#44aa99]/90 disabled:opacity-50 disabled:cursor-not-allowed",
        )}
      >
        {submitting ? (
          <span className="flex items-center justify-center gap-2">
            <Loader2 className="size-4 animate-spin" />
            Submitting...
          </span>
        ) : (
          "Submit Reflection"
        )}
      </button>

      {(!allScoresSet || !forwardActionFilled) && (
        <p className="text-xs text-muted-foreground text-center">
          {!allScoresSet && !forwardActionFilled
            ? "Please answer all questions and provide a forward action."
            : !allScoresSet
              ? "Please answer all questions to submit."
              : "Please provide a forward action to submit."}
        </p>
      )}
    </div>
  );
}
